...
`API_COVERAGE.md` (auto-generated by `tools/audit_coverage.py`) is the source of truth for "does our wrapper expose every input the v23 RPC reference says is settable?" Re-run after any wrapper change that adds or removes a settable field. See [`AUDIT.md`](AUDIT.md) for the methodology, run instructions, false-positive triage, and how to extend the suppressions.
