...
@pytest.mark.asyncio
async def test_list_campaign_drafts_name_contains_escapes_quote(
    campaign_draft_service: CampaignDraftService,
    mock_sdk_client: Any,
    mock_ctx: Context,
) -> None:
    ...
    with patch(
        ...
        await campaign_draft_service.list_campaign_drafts(
            ctx=mock_ctx,
            customer_id="1234567890",
            name_contains="Joe's",
        )
    ...
    call_args = mock_google_ads_service.search.call_args  # type: ignore
    query = call_args[1]["query"]
    assert "campaign_draft.name LIKE '%Joe\\'s%'" in query
