...
| Top-level field coverage | **316 / 316 settable**, **296 / 296 immutable**, **15 / 15 required** (100% on every annotation class) |
| Submessage leaf coverage | **1073 / 1073 (100%)** — every field inside every reachable submessage tree is reachable through some wrapper input |
| Tool-wrapper docstring coverage | **2546 / 2546 (100%)** — every parameter on every MCP tool is documented in its `Args:` block |
| Source agreement | **0 disagreements** — devsite markdown ↔ source `.proto` ↔ local SDK agree on every field annotation, top-level and leaf |
| Sub-type coverage | every oneof member on Ad / Asset / Campaign bidding / criterion is exposed (33 ad-type oneofs, 33 asset-type oneofs, 16 bidding oneofs, 30+ criterion oneofs) |
| Test coverage | 113 test files, **1038 tests**, every public method tested |
| Type errors (pyright) | 0 |

### What "100% coverage" means

This MCP server was verified against three sources triangulated by the audit tool ([`tools/audit_coverage.py`](tools/audit_coverage.py)):

1. The published [Google Ads API v23 reference markdown](https://developers.google.com/google-ads/api/reference/rpc/v23/overview) — the canonical user-facing inventory.
2. The source [`.proto` definitions](https://github.com/googleapis/googleapis/tree/master/google/ads/googleads/v23) on GitHub — for `field_behavior` annotations (Required / Immutable / Output-only / Input-only).
3. The locally-installed Python SDK (`google-ads==30.0.0`) — for the runtime types our wrappers actually use.

Coverage is checked at two levels:

- **Top level.** Every field on every Mutate-able resource is exposed by some wrapper method, with the field's Required / Immutable / Output-only annotation honored. Settable fields can be set on update; immutable fields can only be set on create; output-only fields are excluded from input. Every tool parameter is documented in its `Args:` docstring block.
- **Submessage leaves.** Every leaf field inside every reachable submessage tree (e.g. `Campaign.shopping_setting.merchant_id`, `AdGroupCriterion.listing_group.case_value.product_brand.value`) is reachable through some wrapper — either directly (an explicit param), through a typed-message-passthrough (`Optional[ShoppingSetting]`), or through a dict-passthrough (`Optional[Dict[str, Any]]` routed through `set_optional_submessage` / `extend_repeated_submessages`). Common-proto submessages shared across resources are deduped: when AdGroupAd references an Ad resource, the audit doesn't re-traverse Ad's tree because Ad is its own audit section.

The audit tool fails any time it can't reconcile a field across all three sources, surfaces unreachable leaves with their full dotted path, and is run in this repo on every coverage-affecting change. The full coverage report — every resource, every field, every covering wrapper, every gap — is published as [`API_COVERAGE.md`](API_COVERAGE.md) at the root of this repo.

### Coverage report and audit tool

[`API_COVERAGE.md`](API_COVERAGE.md) is auto-generated by [`tools/audit_coverage.py`](tools/audit_coverage.py) and committed as a user-facing artifact. It lists, for every Mutate-able resource:

- Every field with its annotation, source-agreement status, and the MCP tool(s) / service method(s) that cover it.
- Every reachable submessage leaf with its dotted path and reachability mode (explicit / dict_passthrough / typed_passthrough).
- Any gaps (real or audit-tool detection limits) with full callout paths.

Re-run the audit after any wrapper change:

```bash
# Uses the on-disk cache for devsite md + .proto fetches (default).
uv run python tools/audit_coverage.py

# Force a re-fetch of all reference sources, ignoring cache:
uv run python tools/audit_coverage.py --refresh

# Pin the .proto cross-check to a specific commit SHA for fully
# deterministic runs:
uv run python tools/audit_coverage.py --proto-ref <sha>
```

See [`API_AUDIT.md`](API_AUDIT.md) for the full methodology, run instructions, false-positive triage guide, and instructions for extending the suppression registry.
...
The following types exist in the [Google Ads API v23 proto definitions](https://github.com/googleapis/googleapis/tree/master/google/ads/googleads/v23) but are deprecated by Google. They are exposed by this server (so coverage is genuinely complete and the audit can verify it), but each one's docstring flags it as legacy and recommends the modern replacement. New campaigns should use the replacement; the legacy types are exposed only to support reads/edits of pre-existing entities still in customers' accounts.

| Type | Service | Exposed via | Why Deprecated | Recommended Replacement |
|------|---------|-------------|----------------|--------------------------|
| `TextAd` | [AdService](https://developers.google.com/google-ads/api/reference/rpc/v23/TextAdInfo) | `update_ad(text_ad=...)` (dict-passthrough) | Google stopped accepting new text ads in June 2022 | `ResponsiveSearchAdInfo` via `create_responsive_search_ad` |
| `LegacyResponsiveDisplayAd` | [AdService](https://developers.google.com/google-ads/api/reference/rpc/v23/LegacyResponsiveDisplayAdInfo) | `update_ad(legacy_responsive_display_ad=...)` | Superseded by newer format | `ResponsiveDisplayAdInfo` via `create_responsive_display_ad` |
| `LegacyAppInstallAd` | [AdService](https://developers.google.com/google-ads/api/reference/rpc/v23/LegacyAppInstallAdInfo) | `update_ad(legacy_app_install_ad=...)` | Superseded by newer format | `AppAdInfo` via `create_app_ad` |
| `ShoppingSmartAd` | [AdService](https://developers.google.com/google-ads/api/reference/rpc/v23/ShoppingSmartAdInfo) | `update_ad(shopping_smart_ad=...)` | Replaced by Performance Max campaigns | No direct replacement — use Performance Max |
| `CustomIntent` | [AdGroupCriterionService](https://developers.google.com/google-ads/api/reference/rpc/v23/CustomIntentInfo) | `add_custom_intent_criteria` | Deprecated in favor of custom audiences | `CustomAudienceInfo` via `add_custom_audience_criteria` |
